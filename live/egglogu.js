...
<button class="btn" onclick="showUpgradeModal()" style="background:#fff;color:#dc3545;font-weight:700;padding:12px 32px;font-size:1.1em;border:none;border-radius:8px;cursor:pointer">${t('btn_see_plans')}</button>
</div>`;
}
...
<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:var(--radius);padding:12px;margin-bottom:12px;text-align:left;font-size:.85em;color:#991b1b">
...
<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:var(--radius);padding:10px;margin-bottom:16px;text-align:center;font-size:.85em;color:#1e40af">
...
function saveSuggestions(s){try{localStorage.setItem(SUGGEST_KEY,JSON.stringify(s.slice(-200)));_bugUpdateBadge();}catch(e){}}

function injectBugWidget(){
const fab=document.createElement('div');
fab.id='bug-fab';
fab.innerHTML=`<span style="font-size:20px">🐛</span><span id="bug-badge" style="display:none;position:absolute;top:-4px;right:-4px;background:#f44336;color:#fff;font-size:10px;font-weight:700;min-width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center">0</span>`;
...
const totalPending=unsent.length+unsentSug.length;

let h=`<div style="padding:10px 16px 0;background:linear-gradient(135deg,#ff5722,#d32f2f);color:#fff">
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
<span style="font-weight:700">${isEs?'Feedback':'Feedback'}</span>
<div style="display:flex;align-items:center;gap:8px">
${totalPending?`<span style="background:rgba(255,255,255,.25);padding:2px 8px;border-radius:10px;font-size:11px">${totalPending}</span>`:''}
...
<button onclick="_switchWidgetTab('bugs')" style="flex:1;padding:8px;border:none;border-radius:8px 8px 0 0;font-size:12px;font-weight:600;cursor:pointer;${tabBugs?'background:var(--card,#1a1a2e);color:var(--text,#eee)':'background:transparent;color:rgba(255,255,255,.7)'}">🐛 Bugs${unsent.length?' ('+unsent.length+')':''}</button>
<button onclick="_switchWidgetTab('suggestions')" style="flex:1;padding:8px;border:none;border-radius:8px 8px 0 0;font-size:12px;font-weight:600;cursor:pointer;${!tabBugs?'background:var(--card,#1a1a2e);color:var(--text,#eee)':'background:transparent;color:rgba(255,255,255,.7)'}">💡 ${isEs?'Sugerencias':'Suggestions'}${unsentSug.length?' ('+unsentSug.length+')':''}</button>
</div></div>`;
...
<span><strong>${isEs?'Errores JS':'JS Errors'}:</strong> <span style="color:${errCount?'#f44336':'#4caf50'};font-weight:700">${errCount}</span></span>
</div></div>`;
...
<div style="margin-bottom:8px"><textarea id="bug-desc" rows="2" placeholder="${isEs?'Describe qué pasó...':'Describe what happened...'}" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border,#333);background:var(--bg,#111);color:var(--text,#eee);font-size:12px;resize:vertical;box-sizing:border-box"></textarea></div>
<div style="display:flex;gap:8px">
...
${errCount?`<button onclick="_attachErrors()" style="padding:8px 12px;border:none;border-radius:8px;background:rgba(255,152,0,.2);color:#ff9800;cursor:pointer;font-size:11px;font-weight:600" title="${isEs?'Adjuntar errores capturados':'Attach captured errors'}">📎 ${errCount} err</button>`:''}
</div></div>`;
...
<div>${isEs?'No hay bugs reportados':'No bugs reported'}</div>
<div style="font-size:11px;margin-top:4px">${isEs?'Navega la app y reporta cualquier problema':'Browse the app and report any issues'}</div>
</div>`;
}else{
if(unsent.length>0){
h+=`<div style="padding:6px 16px;background:rgba(255,87,34,.06);font-size:10px;font-weight:700;color:#ff5722;text-transform:uppercase;letter-spacing:.5px">${isEs?'Pendientes de envío':'Pending'} (${unsent.length})</div>`;
}
...
const sentBadge=isSent?`<span style="background:rgba(76,175,80,.15);color:#4caf50;font-size:10px;padding:1px 6px;border-radius:8px;margin-left:4px">✓ ${isEs?'Enviado':'Sent'}</span>`:`<span style="background:rgba(255,152,0,.15);color:#ff9800;font-size:10px;padding:1px 6px;border-radius:8px;margin-left:4px">${isEs?'Pendiente':'Pending'}</span>`;
...
if(b.errors?.length){
h+=`<details style="margin-top:4px"><summary style="font-size:10px;color:#f44336;cursor:pointer">${isEs?'Ver errores adjuntos':'View attached errors'} (${b.errors.length})</summary>
...
${!isSent?`<button onclick="_sendBug(${idx})" style="background:none;border:1px solid rgba(76,175,80,.4);color:#4caf50;cursor:pointer;font-size:11px;padding:2px 8px;border-radius:4px;white-space:nowrap" title="${isEs?'Enviar':'Send'}">📨</button>`:''}
<button onclick="_deleteBug(${idx})" style="background:none;border:none;color:var(--text-muted,#666);cursor:pointer;font-size:14px;padding:2px;text-align:center" title="${isEs?'Eliminar':'Delete'}">🗑</button>
</div>
</div></div>`;
});
}
...
<span style="font-size:11px;color:var(--text-muted,#666)">${bugs.length} bug${bugs.length!==1?'s':''} · ${unsent.length} ${isEs?'pendiente':'pending'}${unsent.length!==1?'s':''}</span>
<div style="display:flex;gap:6px;flex-wrap:wrap">
${unsent.length>0?`<button onclick="_sendAllBugs()" style="padding:4px 10px;border:none;border-radius:6px;background:linear-gradient(135deg,#4caf50,#388e3c);color:#fff;cursor:pointer;font-size:11px;font-weight:600">📨 ${isEs?'Enviar':'Send'} ${unsent.length>1?(isEs?'todos':'all')+' ('+unsent.length+')':'1'}</button>`:''}
<button onclick="_exportBugs()" style="padding:4px 10px;border:none;border-radius:6px;background:rgba(255,255,255,.08);color:var(--text,#eee);cursor:pointer;font-size:11px">📋 ${isEs?'Exportar':'Export'}</button>
<button onclick="_clearBugs()" style="padding:4px 10px;border:none;border-radius:6px;background:rgba(244,67,54,.1);color:#f44336;cursor:pointer;font-size:11px">🗑 ${isEs?'Limpiar':'Clear'}</button>
</div></div>`;
}
}else{
// ——— SUGGESTIONS TAB ———
...
h+=`<div style="text-align:center;padding:30px 16px;color:var(--text-muted,#666)">
<div style="font-size:32px;margin-bottom:8px">💡</div>
<div>${isEs?'No hay sugerencias aún':'No suggestions yet'}</div>
<div style="font-size:11px;margin-top:4px">${isEs?'Comparte ideas para mejorar la app':'Share ideas to improve the app'}</div>
</div>`;
}else{
if(unsentSug.length>0){
h+=`<div style="padding:6px 16px;background:rgba(124,77,255,.06);font-size:10px;font-weight:700;color:#7c4dff;text-transform:uppercase;letter-spacing:.5px">${isEs?'Pendientes de envío':'Pending'} (${unsentSug.length})</div>`;
}
...
const sentBadge=isSent?`<span style="background:rgba(76,175,80,.15);color:#4caf50;font-size:10px;padding:1px 6px;border-radius:8px;margin-left:4px">✓ ${isEs?'Enviado':'Sent'}</span>`:`<span style="background:rgba(124,77,255,.15);color:#7c4dff;font-size:10px;padding:1px 6px;border-radius:8px;margin-left:4px">${isEs?'Pendiente':'Pending'}</span>`;
const opacity=isSent?'0.55':'1';
h+=`<div style="padding:10px 16px;border-bottom:1px solid var(--border,#222);opacity:${opacity}">
<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px">
<div style="flex:1;min-width:0">
<div style="font-size:11px;color:var(--text-muted,#666);margin-bottom:3px">${ts} · <code style="font-size:10px;background:rgba(255,255,255,.06);padding:1px 5px;border-radius:4px">${sanitizeHTML(s.section)}</code> ${sentBadge}</div>
<div style="font-size:12px;line-height:1.4">${catIcon} ${sanitizeHTML(s.desc)}</div>
</div>
<div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
${!isSent?`<button onclick="_sendSuggestion(${idx})" style="background:none;border:1px solid rgba(76,175,80,.4);color:#4caf50;cursor:pointer;font-size:11px;padding:2px 8px;border-radius:4px;white-space:nowrap" title="${isEs?'Enviar':'Send'}">📨</button>`:''}
<button onclick="_deleteSuggestion(${idx})" style="background:none;border:none;color:var(--text-muted,#666);cursor:pointer;font-size:14px;padding:2px;text-align:center" title="${isEs?'Eliminar':'Delete'}">🗑</button>
</div>
</div></div>`;
});
}
...
if(suggestions.length>0){
h+=`<div style="padding:8px 16px;border-top:1px solid var(--border,#333);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">
<span style="font-size:11px;color:var(--text-muted,#666)">${suggestions.length} ${isEs?'sugerencia':'suggestion'}${suggestions.length!==1?'s':''} · ${unsentSug.length} ${isEs?'pendiente':'pending'}${unsentSug.length!==1?'s':''}</span>
<div style="display:flex;gap:6px;flex-wrap:wrap">
${unsentSug.length>0?`<button onclick="_sendAllSuggestions()" style="padding:4px 10px;border:none;border-radius:6px;background:linear-gradient(135deg,#4caf50,#388e3c);color:#fff;cursor:pointer;font-size:11px;font-weight:600">📨 ${isEs?'Enviar':'Send'} ${unsentSug.length>1?(isEs?'todas':'all')+' ('+unsentSug.length+')':'1'}</button>`:''}
<button onclick="_exportSuggestions()" style="padding:4px 10px;border:none;border-radius:6px;background:rgba(255,255,255,.08);color:var(--text,#eee);cursor:pointer;font-size:11px">📋 ${isEs?'Exportar':'Export'}</button>
<button onclick="_clearSuggestions()" style="padding:4px 10px;border:none;border-radius:6px;background:rgba(244,67,54,.1);color:#f44336;cursor:pointer;font-size:11px">🗑 ${isEs?'Limpiar':'Clear'}</button>
</div></div>`;
}
}
p.innerHTML=h;
