...
def upgrade() -> None:
    ...
    op.execute("DO $$ BEGIN CREATE TYPE role AS ENUM ('owner','manager','vet','viewer'); EXCEPTION WHEN duplicate_object THEN null; END $$")
    op.execute("DO $$ BEGIN CREATE TYPE stresstype AS ENUM ('heat','cold','noise','predator','handling','other'); EXCEPTION WHEN duplicate_object THEN null; END $$")
    ...
    sa.Column('role', sa.Enum('owner', 'manager', 'vet', 'viewer', name='role', create_type=False), nullable=False),
    sa.Column('organization_id', sa.Uuid(), nullable=False),
    sa.Column('is_active', sa.Boolean(), nullable=False),
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.DateTime(), server_default=sa.text('now()'), nullable=False),
    ...
    sa.Column('flock_id', sa.Uuid(), nullable=False),
    sa.Column('date', sa.Date(), nullable=False),
    sa.Column('type', sa.Enum('heat', 'cold', 'noise', 'predator', 'handling', 'other', name='stresstype', create_type=False), nullable=False),
