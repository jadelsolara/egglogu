...
def upgrade() -> None:
    ...
    op.execute("DO $$ BEGIN CREATE TYPE eggtype AS ENUM ('conventional','free_range','organic','pasture_raised','decorative'); EXCEPTION WHEN duplicate_object THEN null; END $$")
    op.execute("DO $$ BEGIN CREATE TYPE marketchannel AS ENUM ('wholesale','supermarket','restaurant','direct','export','pasteurized'); EXCEPTION WHEN duplicate_object THEN null; END $$")
    ...
    # ── Subscriptions ──
    op.execute("DO $$ BEGIN CREATE TYPE plantier AS ENUM ('free','pro','business','enterprise'); EXCEPTION WHEN duplicate_object THEN null; END $$")
    op.execute("DO $$ BEGIN CREATE TYPE subscriptionstatus AS ENUM ('active','cancelled','past_due'); EXCEPTION WHEN duplicate_object THEN null; END $$")
