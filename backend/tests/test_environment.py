...
class TestListEnvironment:
    async def test_list_environment_empty(
        self, client: AsyncClient, authenticated_user
    ):
        response = await client.get(
            f"{API}/environment", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_environment_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
            f"{API}/environment",
            json={"date": "2025-01-01", "temp_c": 25.0, "humidity_pct": 60},
            headers=headers,
        )
        await client.post(
            f"{API}/environment",
            json={"date": "2025-01-02", "temp_c": 26.0, "humidity_pct": 65},
            headers=headers,
        )
...
class TestCreateEnvironment:
    async def test_create_environment_minimal(
        ...
        response = await client.post(
            f"{API}/environment", json=payload, headers=headers
        )
...
class TestDeleteEnvironment:
    ...
    async def test_delete_environment_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
...
class TestListIoT:
    async def test_list_iot_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/iot-readings", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_iot_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
...
class TestCreateIoT:
    async def test_create_iot_minimal(self, client: AsyncClient, authenticated_user):
        ...
        response = await client.post(
            f"{API}/iot-readings", json=payload, headers=headers
        )
        assert response.status_code == 201
        data = response.json()
        assert "id" in data
...
class TestDeleteIoT:
    ...
    async def test_delete_iot_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
        fake_id = str(uuid.uuid4())
        response = await client.delete(
...
class TestListWeather:
    async def test_list_weather_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/weather", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []
