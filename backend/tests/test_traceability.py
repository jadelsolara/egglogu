...
async def _create_farm_and_flock(client: AsyncClient, headers: dict) -> tuple[str, str]:
    ...
    farm_resp = await client.post(
        FARM_PREFIX, json={"name": "Trace Farm"}, headers=headers
    )
...
class TestListBatches:
    async def test_list_batches_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{PREFIX}/batches", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_batches_returns_created(
...
class TestDeleteBatch:
    ...
    async def test_delete_batch_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
