...
class TestPagination:
    async def test_endpoints_accept_page_size(
        ...
        for endpoint in PAGINATED_ENDPOINTS:
            response = await client.get(f"{endpoint}?page=1&size=10", headers=headers)
    ...
    async def test_pagination_invalid_page(
        ...
        headers = authenticated_user["headers"]
        response = await client.get("/api/v1/farms/?page=0&size=10", headers=headers)
        assert response.status_code == 422

    async def test_pagination_size_limit(self, client: AsyncClient, authenticated_user):
        """Size > 200 should return 422."""
        headers = authenticated_user["headers"]
        response = await client.get("/api/v1/farms/?page=1&size=500", headers=headers)
        assert response.status_code == 422
