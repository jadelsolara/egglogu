...
class TestListChecklist:
    async def test_list_checklist_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/checklist", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_checklist_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
            f"{API}/checklist", json={"label": "Check water"}, headers=headers
        )
        await client.post(
            f"{API}/checklist", json={"label": "Check feed"}, headers=headers
        )
...
class TestCreateChecklist:
    async def test_create_checklist_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
...
class TestDeleteChecklist:
    async def test_delete_checklist_success(
        ...
        create_resp = await client.post(
            f"{API}/checklist", json={"label": "Doomed item"}, headers=headers
        )
    ...
    async def test_delete_checklist_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
...
class TestListLogbook:
    async def test_list_logbook_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/logbook", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_logbook_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
            f"{API}/logbook",
            json={"date": "2025-01-01", "text": "All normal"},
            headers=headers,
        )
        await client.post(
            f"{API}/logbook",
            json={"date": "2025-01-02", "text": "Rainy day"},
            headers=headers,
        )
...
class TestCreateLogbook:
    async def test_create_logbook_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
    ...
    async def test_create_logbook_unauthenticated(self, client: AsyncClient):
        response = await client.post(
            f"{API}/logbook", json={"date": "2025-01-01", "text": "All normal"}
        )
        assert response.status_code == 401
...
class TestDeleteLogbook:
    async def test_delete_logbook_success(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        create_resp = await client.post(
            f"{API}/logbook",
            json={"date": "2025-01-01", "text": "Doomed entry"},
            headers=headers,
        )
        item_id = create_resp.json()["id"]
    ...
    async def test_delete_logbook_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
        fake_id = str(uuid.uuid4())
        response = await client.delete(
...
class TestListPersonnel:
    async def test_list_personnel_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/personnel", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_personnel_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
            f"{API}/personnel",
            json={"name": "John", "role": "caretaker"},
            headers=headers,
        )
        await client.post(
            f"{API}/personnel",
            json={"name": "Jane", "role": "manager"},
            headers=headers,
        )
...
class TestCreatePersonnel:
    async def test_create_personnel_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
    ...
    async def test_create_personnel_unauthenticated(self, client: AsyncClient):
        response = await client.post(
            f"{API}/personnel", json={"name": "John", "role": "caretaker"}
        )
        assert response.status_code == 401
...
class TestDeletePersonnel:
    async def test_delete_personnel_success(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        create_resp = await client.post(
            f"{API}/personnel",
            json={"name": "Doomed Worker", "role": "caretaker"},
            headers=headers,
        )
        item_id = create_resp.json()["id"]
    ...
    async def test_delete_personnel_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
        fake_id = str(uuid.uuid4())
        response = await client.delete(
