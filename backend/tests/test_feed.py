...
async def _create_farm_and_flock(client: AsyncClient, headers: dict) -> tuple[str, str]:
    ...
    farm_resp = await client.post(
        FARM_PREFIX, json={"name": "Feed Farm"}, headers=headers
    )
    assert farm_resp.status_code == 201
    farm_id = farm_resp.json()["id"]

    flock_resp = await client.post(
        FLOCK_PREFIX,
        json={
            "farm_id": farm_id,
            "name": "Feed Flock",
...
class TestListPurchases:
    async def test_list_purchases_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{PREFIX}/purchases", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_purchases_returns_created(
...
class TestCreatePurchase:
    async def test_create_purchase_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        payload = {
        ...
        response = await client.post(
            f"{PREFIX}/purchases", json=payload, headers=headers
        )
...
class TestDeletePurchase:
    ...
    async def test_delete_purchase_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
...
class TestListConsumption:
    async def test_list_consumption_empty(
        self, client: AsyncClient, authenticated_user
    ):
        response = await client.get(
            f"{PREFIX}/consumption", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_consumption_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        _, flock_id = await _create_farm_and_flock(client, headers)
        await client.post(
            f"{PREFIX}/consumption",
            json={"flock_id": flock_id, "date": "2025-01-01", "feed_kg": 50.0},
            headers=headers,
        )
        await client.post(
            f"{PREFIX}/consumption",
            json={"flock_id": flock_id, "date": "2025-01-02", "feed_kg": 55.0},
            headers=headers,
        )
...
class TestCreateConsumption:
    async def test_create_consumption_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        _, flock_id = await _create_farm_and_flock(client, headers)
        ...
        response = await client.post(
            f"{PREFIX}/consumption", json=payload, headers=headers
        )
        assert response.status_code == 201
        data = response.json()
        assert "id" in data
    ...
    async def test_create_consumption_unauthenticated(self, client: AsyncClient):
        response = await client.post(
            f"{PREFIX}/consumption",
            json={"flock_id": str(uuid.uuid4()), "date": "2025-01-01", "feed_kg": 50.0},
        )
        assert response.status_code == 401
...
class TestDeleteConsumption:
    async def test_delete_consumption_success(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        _, flock_id = await _create_farm_and_flock(client, headers)
        ...
        del_resp = await client.delete(
            f"{PREFIX}/consumption/{item_id}", headers=headers
        )
        assert del_resp.status_code == 204
    ...
    async def test_delete_consumption_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
        fake_id = str(uuid.uuid4())
        response = await client.delete(
