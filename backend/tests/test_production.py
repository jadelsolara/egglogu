...
async def _create_farm_and_flock(client: AsyncClient, headers: dict) -> tuple[str, str]:
    ...
    farm_resp = await client.post(
        FARM_PREFIX, json={"name": "Prod Farm"}, headers=headers
    )
...
class TestListProduction:
    ...
    async def test_list_production_returns_created(
        ...
        await client.post(
            PREFIX,
            json={"flock_id": flock_id, "date": "2025-01-15", "total_eggs": 500},
            headers=headers,
        )
        await client.post(
            PREFIX,
            json={"flock_id": flock_id, "date": "2025-01-16", "total_eggs": 600},
            headers=headers,
        )
...
class TestCreateProduction:
    async def test_create_production_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        _, flock_id = await _create_farm_and_flock(client, headers)
...
class TestDeleteProduction:
    async def test_delete_production_success(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        _, flock_id = await _create_farm_and_flock(client, headers)
    ...
    async def test_delete_production_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
