...
async def _create_client(client: AsyncClient, headers: dict) -> str:
    ...
    resp = await client.post(
        CLIENT_PREFIX, json={"name": "Finance Client"}, headers=headers
    )
...
class TestListIncome:
    async def test_list_income_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/income", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_income_returns_created(
...
class TestDeleteIncome:
    ...
    async def test_delete_income_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
...
class TestListExpenses:
    async def test_list_expenses_empty(self, client: AsyncClient, authenticated_user):
        response = await client.get(
            f"{API}/expenses", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []

    async def test_list_expenses_returns_created(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        await client.post(
...
class TestCreateExpense:
    async def test_create_expense_minimal(
        self, client: AsyncClient, authenticated_user
    ):
        headers = authenticated_user["headers"]
        payload = {
...
class TestDeleteExpense:
    ...
    async def test_delete_expense_nonexistent(
        self, client: AsyncClient, authenticated_user
    ):
        fake_id = str(uuid.uuid4())
        response = await client.delete(
...
class TestListReceivables:
    async def test_list_receivables_empty(
        self, client: AsyncClient, authenticated_user
    ):
        response = await client.get(
            f"{API}/receivables", headers=authenticated_user["headers"]
        )
        assert response.status_code == 200
        assert response.json() == []
