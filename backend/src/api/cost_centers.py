...
from src.models.cost_center import (
    CostCenter, CostAllocation, ProfitLossSnapshot,
)
from src.schemas.cost_center import (
    CostCenterCreate, CostCenterUpdate, CostCenterRead,
    CostAllocationCreate, CostAllocationUpdate, CostAllocationRead,
    ProfitLossSnapshotCreate, ProfitLossSnapshotRead,
)
...
async def list_cost_centers(
    ...
    stmt = (
        select(CostCenter)
        .where(CostCenter.organization_id == user.organization_id)
    )
    if center_type:
...
@router.post("/allocations", response_model=CostAllocationRead, status_code=status.HTTP_201_CREATED)
...
@router.post("/{center_id}/pl", response_model=ProfitLossSnapshotRead, status_code=status.HTTP_201_CREATED)
async def create_pl_snapshot(
    ...
    margin_pct = (gross_profit / data.total_revenue * 100) if data.total_revenue > 0 else 0.0
    cost_per_egg = (data.total_cost / data.eggs_produced) if data.eggs_produced else None
...
async def cost_center_overview(
    ...
    for center in centers:
        ...
        cost_result = await db.execute(
            select(func.sum(CostAllocation.amount))
            .where(CostAllocation.cost_center_id == center.id)
        )
        ...
        overview.append({
            ...
            "name": center.name,
            "code": center.code,
            "center_type": center.center_type.value if hasattr(center.center_type, 'value') else center.center_type,
            "budget_monthly": center.budget_monthly,
            "total_allocated": total_allocated,
            "budget_utilization_pct": round(
                (total_allocated / center.budget_monthly * 100), 1
            ) if center.budget_monthly and center.budget_monthly > 0 else None,
        })
    return overview
