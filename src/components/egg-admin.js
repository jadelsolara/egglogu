...
class EggAdmin extends HTMLElement {
  ...
  _showUpgradeModal() {
    ...
    if (!isPaying) {
      body += `<div style="display:inline-block;background:#dcfce7;color:#166534;padding:4px 14px;border-radius:12px;font-size:.85rem;margin-bottom:16px">40% off \u2014 ${lbl.first_3_months || 'first 3 months'}</div>`;
    }
    ...
    if (!isPaying) {
      body += `<p style="font-size:.78em;color:var(--text-muted);margin:0">${lbl.discount_note || 'Welcome discount decreases gradually'}</p>`;
    }
