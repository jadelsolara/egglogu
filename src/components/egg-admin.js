...
class EggAdmin extends HTMLElement {
  ...
  render() {
    ...
    // ── 1. HERO KPI STRIP — 4 key numbers at a glance ──
    h += `<div class="hero-strip">
    <div class="hero-kpi">
      <div class="hero-value" style="color:var(--primary)">$${totalMRR.toFixed(2)}</div>
      <div class="hero-label">${lbl.total_mrr}</div>
    </div>
    <div class="hero-kpi">
      <div class="hero-value" style="color:#4caf50">${activeUsers.length}<small style="font-size:0.5em;color:var(--text-muted)">/${users.length}</small></div>
      <div class="hero-label">${lbl.active_users}</div>
    </div>
    <div class="hero-kpi">
      <div class="hero-value" style="color:${retention >= 90 ? '#4caf50' : retention >= 75 ? '#ff9800' : '#f44336'}">${retention}%</div>
      <div class="hero-label">${lbl.retention}</div>
    </div>
    <div class="hero-kpi">
      <div class="hero-value" style="color:${pending.length ? '#ff9800' : '#4caf50'}">${pending.length}</div>
      <div class="hero-label">${lbl.pending_activations}</div>
    </div>
    </div>`;

    // ── 2. PLAN BAR — compact horizontal strip ──
    const maxFlocksStr = tierInfo.maxFlocks === Infinity ? '\u221E' : tierInfo.maxFlocks;
    const maxUsersStr = tierInfo.includedUsers === Infinity ? '\u221E' : tierInfo.includedUsers;
    h += `<div class="plan-bar">
    <div class="plan-bar-left">
      <span class="plan-tier">${tierInfo.name.toUpperCase()}</span>
      <span class="plan-detail">$${tierInfo.baseCost}/${cur} &middot; ${maxFlocksStr} ${lbl.flocks || 'flocks'} &middot; ${maxUsersStr} ${lbl.users || 'users'} &middot; ${billingCycle}</span>
    </div>
    <div class="plan-bar-right">
      <span style="font-size:0.8rem;color:var(--text-muted)">${lbl.billing_next}: ${nextBilling}</span>
      <button class="btn btn-sm" data-action="upgrade-plan">${lbl.upgrade_plan}</button>
    </div>
    </div>`;
    ...
    // ── LEFT: User Table ──
    h += `<div class="admin-col-main">`;
    h += `<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
    <h3 style="margin:0">${lbl.user_table} <span style="font-size:0.75rem;color:var(--text-muted);font-weight:400">(${activeUsers.length} ${lbl.active.toLowerCase()}, ${inactiveUsers.length} ${lbl.inactive.toLowerCase()})</span></h3>
    ...
    if (users.length === 0) {
      ...
      // Inline role breakdown
      const roleStr = Object.entries(roleCounts).map(([r, c]) => `${r}: ${c}`).join(' &middot; ');
      if (roleStr) h += `<div style="font-size:0.8rem;color:var(--text-muted);margin:8px 0 4px">${lbl.by_role}: ${roleStr}</div>`;

      h += `<div style="overflow-x:auto"><table class="data-table" style="width:100%;margin-top:8px"><thead><tr>
      <th>${lbl.name}</th><th>${lbl.email}</th><th>${lbl.role}</th><th>${lbl.status}</th><th>${lbl.since}</th><th style="min-width:160px">${lbl.actions}</th>
      </tr></thead><tbody>`;
      users.forEach(u => {
    ...
    h += `</div>`;
    h += `</div>`; // end admin-col-main

    // ── RIGHT: Billing + Revenue + Team Metrics ──
    h += `<div class="admin-col-side">`;

    // Revenue summary
    h += `<div class="card">
    <h3>${lbl.revenue_kpis}</h3>
    <div class="side-metric"><span>${lbl.base_revenue}</span><strong>$${baseRevenue.toFixed(2)}</strong></div>
    <div class="side-metric"><span>${lbl.extra_user_revenue} (${extraUsers})</span><strong>$${extraRevenue.toFixed(2)}</strong></div>
    <div class="side-metric side-metric-total"><span>${lbl.billing_total}</span><strong style="color:var(--primary);font-size:1.2rem">$${totalMonthlyCharge.toFixed(2)}</strong></div>
    </div>`;

    // Billing details
    h += `<div class="card">
    <h3>${lbl.billing_title}</h3>
    <div class="side-metric"><span>${lbl.billing_cycle}</span><strong style="text-transform:capitalize">${billingCycle}</strong></div>
    <div class="side-metric"><span>${lbl.billing_base}</span><strong>$${tierInfo.baseCost.toFixed(2)}</strong></div>
    <div class="side-metric"><span>${lbl.billing_extra} (${extraUsersActive.length})</span><strong>$${totalExtraCharge.toFixed(2)}</strong></div>
    <div style="margin-top:8px"><button class="btn btn-sm" data-action="billing-portal" style="width:100%;text-align:center">${lbl.manage || 'Manage'}</button></div>
    </div>`;

    // Team metrics (activation + churn combined)
    h += `<div class="card">
    <h3>${lbl.team_metrics || 'Team Metrics'}</h3>
    <div class="side-metric"><span>${lbl.activation_rate}</span><strong>${activationRate}%</strong></div>
    <div class="side-metric"><span>${lbl.avg_time_activate}</span><strong>${avgActivationDays} ${lbl.days}</strong></div>
    <div class="side-metric"><span>${lbl.monthly_churn}</span><strong style="color:${churnRate > 10 ? '#f44336' : churnRate > 5 ? '#ff9800' : '#4caf50'}">${churnRate}%</strong></div>
    <div class="side-metric"><span>${lbl.deactivated_30d}</span><strong>${deactivated30d.length}</strong></div>
    </div>`;

    h += `</div>`; // end admin-col-side
    h += `</div>`; // end admin-grid

    // ── 4. PAYMENT LEDGER (full width, collapsed by default) ──
    const ledger = [];
    users.forEach(u => {
    ...
    if (ledger.length > 0) {
      h += `<div class="card"><h3>${lbl.billing_ledger}</h3>
      ...
      h += `</tbody></table></div>
      <div style="margin-top:8px;padding:8px;background:rgba(76,175,80,.06);border-radius:var(--radius);font-size:0.75rem;color:var(--text-muted)">${lbl.billing_auto_note}</div>
      </div>`;
    }

    // ── 5. AUDIT LOG — compact ──
    h += `<div class="card"><h3>${lbl.audit_recent}</h3>`;
    const recentAudit = audit.slice(-15).reverse();
    if (recentAudit.length === 0) {
      h += `<p style="color:var(--text-muted);text-align:center;padding:16px">${lbl.no_audit}</p>`;
    } else {
      h += `<div style="overflow-x:auto;max-height:200px;overflow-y:auto"><table class="data-table" style="width:100%;font-size:0.8rem"><thead><tr>
      <th>${lbl.date}</th><th>${lbl.name}</th><th>${lbl.actions}</th><th>${lbl.billing_desc}</th>
      </tr></thead><tbody>`;
      recentAudit.forEach(a => {
        ...
        <td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${sanitizeHTML(a.detail || '\u2014')}</td>
        </tr>`;
      });
      h += `</tbody></table></div>`;
    }
